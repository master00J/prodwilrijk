...
export async function fetchPrepackStats({
  ...
  // Haal ook airtec-logs op voor overlapping-detectie (zelfde periode, afgesloten logs)
  const airtecTimeLogs = await fetchAllRows<any>(async (from, to) => {
    let query = supabaseAdmin
      .from('time_logs')
  ...
  const dailyStatsArray: DailyStat[] = Object.values(dailyStats)
    .map((stat) => {
      ...
      // Als er in het weekend gewerkt wordt (expectedHours = 0 maar er zijn manuren),
      // hanteren we een standaard werkdag van 8u als FTE-basis zodat zaterdag/zondag-
      // werk correct wordt meegerekend in FTE en items/FTE.
