...
export default function LabelScanner({ onItemsMatched, onUnlistedAdded }: LabelScannerProps) {
  ...
  const processItem = useCallback(async (item: QueueItem) => {
    ...
    try {
      ...
      if (data.labelType === 'powertools') {
        const addRes = await fetch('/api/incoming-goods/scan-label/add-to-pack', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            item_number: data.label.item_number,
            po_number: data.label.po_line,
            amount: data.label.quantity || 1,
          }),
        })
        const addData = await addRes.json()
        const msg = addData.skipped
          ? `Al in Items to Pack`
          : `Toegevoegd aan Items to Pack`
        setQueue(prev => prev.map(q => q.id === item.id ? { ...q, status: 'done', result: data, autoAction: msg } : q))
        return
      }

      if (data.labelType === 'd_nummer') {
        await fetch('/api/incoming-goods/unlisted', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            item_number: data.label.item_number,
            ...
            po_line: data.label.po_line,
            supplier: data.label.supplier,
            label_date: data.label.date,
            delivery_notice: data.label.delivery_notice,
            category: 'd_nummer',
            opmerking: `D-nummer: ${data.label.delivery_notice}`,
          }),
        })
        onUnlistedAdded()
        setQueue(prev => prev.map(q => q.id === item.id ? { ...q, status: 'done', result: data, autoAction: `D-nummer ${data.label.delivery_notice} → lijst` } : q))
        return
      }
      ...
      if (data.matches.length > 0 && data.label.item_number) {
        ...
        // Small delay to let setScanTally resolve before checking isExtra
  ...
  }, [onItemsMatched, onUnlistedAdded])
...
function ScanCard({ item, onAddToUnlisted }: { item: QueueItem; onAddToUnlisted: () => void }) {
  const label = item.result?.label
  const labelType = item.result?.labelType
  ...
  const typeLabel = labelType === 'powertools' ? 'Powertools'
    : labelType === 'd_nummer' ? 'D-nummer'
    : 'Prepack'

  const typeColor = labelType === 'powertools' ? 'bg-emerald-100 text-emerald-700'
    : labelType === 'd_nummer' ? 'bg-purple-100 text-purple-700'
        ...
        <div className="flex-1 min-w-0">
          ...
          {label && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {labelType && (
                <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${typeColor}`}>{typeLabel}</span>
              )}
              {label.description && (
                <span className="text-xs text-gray-500 truncate max-w-[150px]">{label.description}</span>
              )}
            </div>
          )}
