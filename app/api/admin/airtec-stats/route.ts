...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get time logs for items_to_pack_airtec type
    ...
    // Process packed items (Airtec uses 'quantity' instead of 'amount')
    items.forEach((item) => {
      ...
      dailyStats[date].itemsPacked += item.quantity || 0
    })
    ...
    // Calculate totals
    const totalItemsPacked = items.reduce((sum, item) => sum + (item.quantity || 0), 0)
      ...
      if (dayWork) {
        ...
        if (totalHoursForDay > 0) {
          Object.entries(dayWork).forEach(([personName, hours]) => {
            ...
            personStatsMap[personName].itemsPacked += Math.round((item.quantity || 0) * proportion)
          })
        }
      }
    })
