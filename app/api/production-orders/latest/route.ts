...
export async function GET(_request: NextRequest) {
  try {
    ...
    if (!order) {
      return NextResponse.json({ order: null, lines: [], materials: [], totals: {} })
    }
    ...
    const linesWithCost = (lines || []).map((line: any) => {
      ...
      components.forEach((component: any) => {
        componentCount += 1
        const itemNo = component.component_item_no
        const unit = Number(component.component_unit) || 0
    ...
    return NextResponse.json(
      { error: error.message || 'Fout bij ophalen productieorder' },
      { status: 500 }
    )
  }
}
