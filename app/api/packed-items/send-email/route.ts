...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `packed_items_${dateFromStr}_to_${dateToStr}.xlsx`

    // Send email
    await transporter.sendMail({
      from: emailConfig.from,
      to: email,
      subject: `Packed Items Report - ${dateFromStr} to ${dateToStr}`,
      text: `Please find attached the packed items report for the date range ${dateFromStr} to ${dateToStr}.\n\nTotal items: ${items.length}`,
      html: `
        <p>Please find attached the packed items report for the date range <strong>${dateFromStr}</strong> to <strong>${dateToStr}</strong>.</p>
